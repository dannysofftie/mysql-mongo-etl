...
export function mongoConfig(options?: IMongoConfig): string {
    try {
        return new RegExp(/^(?:(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(\.(?!$)|$)){4}$/).test(options.mongohost)
            ? `mongodb://${options.mongousername.trim()}:${options.mongopassword.trim()}@${options.mongohost.trim()}}?retryWrites=true`
            : `mongodb+srv://${options.mongousername.trim()}:${options.mongopassword.trim()}@${options.mongohost.trim()}}?retryWrites=true`;
    } catch (e) {
        return process.exit();
    }
}
